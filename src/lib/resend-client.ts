...
export function validateResendConfig(): { valid: boolean; missing: string[] } {
  ...
  if (!RESEND_CONFIG.fromEmail || RESEND_CONFIG.fromEmail === "onboarding@resend.dev") {
    missing.push("RESEND_FROM_EMAIL");
  }
  if (!RESEND_CONFIG.toEmail || RESEND_CONFIG.toEmail === "delivered@resend.dev") {
    missing.push("RESEND_TO_EMAIL");
  }
