...
function validateStringField(value: unknown, minLength: number, maxLength: number): boolean {
...
export function validateContactForm(data: unknown): { valid: boolean; errors: string[] } {
  ...
  if (typeof formData.email !== "string" || formData.email.length > 100 || !validateEmail(formData.email)) errors.push("Email invalide ou trop long");
